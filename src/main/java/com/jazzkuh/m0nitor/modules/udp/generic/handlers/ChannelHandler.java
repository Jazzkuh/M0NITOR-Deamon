...
public class ChannelHandler extends GenericHandler {
    ...
    public boolean shouldProcess(byte size, byte cmd) {
        return size == (byte) 0x0A && (cmd == (byte) 0xE0 || cmd == (byte) 0xA0);
    }
    ...
    public void process(byte size, byte cmd, byte[] data) {
        for (int i = 1; i <= 8; i++) {
            ...
            boolean channelOn = data[fader.index()] >= 1;

            if (fader.isChannelOn() == channelOn) continue;
            fader.setChannelOn(channelOn);

            ChannelTrigger channelTrigger = new ChannelTrigger(i, channelOn ? TriggerType.CHANNEL_ON : TriggerType.CHANNEL_OFF);
            if (channelOn && fader.isFaderActive()) {
