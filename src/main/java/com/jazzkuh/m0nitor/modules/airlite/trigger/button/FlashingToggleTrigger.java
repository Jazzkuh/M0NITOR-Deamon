...
public class FlashingToggleTrigger extends TriggerAction {
	...
	public void process() {
		...
		if (!airliteModule.getEnabledButtons().contains("flashing")) {
			udpModule.writeBlinkingLed(ControlButton.LED_6A, ControlLedColor.GREEN, ControlLedColor.OFF, ControlLedBlinkSpeed.SLOW);
			airliteModule.getEnabledButtons().add("flashing");
		} else {
			udpModule.writeStaticLed(ControlButton.LED_6A, ControlLedColor.GREEN);
